...
**Note:** Limitations listed above only apply when submitting a new rule to core ESLint repository. If you are working on a custom rule or plugin, naming conventions and availability of unit tests are up to you.
...
* fixable - String explaining what type of fix this rule supports. Can be `whitespace` or `functional`. Omit this property if the rule is not fixable
